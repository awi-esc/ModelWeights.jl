@testset "Test getAllCombinations" begin
    ...
    result1 = sort(ModelWeights.getAllCombinations(v1, v2))
    @test result1 == expected1

    # one/two input vectors empty -> should throw warning
    @test_logs (:warn, "At least one input vector is empty -> empty vector returned!") ModelWeights.getAllCombinations([], [])
    @test_logs (:warn, "At least one input vector is empty -> empty vector returned!") ModelWeights.getAllCombinations(["a"], [])

    # unequal number of arguments (second more)
    v1 = ["pr", "psl"]; v2 = ["CLIM", "std", "CLIM-mon"]
    result2 = sort(ModelWeights.getAllCombinations(v1, v2))
